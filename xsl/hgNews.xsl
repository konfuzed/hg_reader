<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:php="http://php.net/xsl" exclude-result-prefixes="php">
  ...
  <xsl:template match="/node">
      ...
      <!-- Byline -->
      <p><em>By <xsl:value-of select="concat(uid/user/profile_firstname, ' ', uid/user/profile_lastname)"/>
      | Posted <xsl:value-of select="php:functionString('hg_reader_XSLDateTimeXfer','fullDateString',field_release/item/value)" /> 
      <xsl:choose>
        <xsl:when test="field_location/item/value = ''">
          | <strong>Atlanta</strong>
        </xsl:when>
        <xsl:otherwise>
          | <strong><xsl:value-of select="field_location/item/value" /></strong>
        </xsl:otherwise>
      </xsl:choose>
      </em></p>
                  ...
                  <xsl:when test="nid/node/type='image'">
                    ...
                    <li class="clearfix">
                      <a href="{$base_path}hg/image/{nid/node/nid}" title="{nid/node/title}">
                        <img src="{$base_path}hg/image/{nid/node/nid}&amp;f=thumbnail_scaled" alt="" />
                      </a> 
                      <p>
                  ...
                  <xsl:otherwise>
                    ...
                    <xsl:if test="nid/node/field_video/item/fid">
                      <li class="clearfix">
                        ...
                        * Note that the below code is using the hgFile function to serve the video file for download.
